...
@Client.on_callback_query(filters.regex(r"^nextch"))
async def next_CHpage(bot, query):
    ident, req, key, offset = query.data.split("_")
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer(script.ALRT_TXT.format(query.from_user.first_name), show_alert=True)
    try:
                ...
                if settings['max_btn']:
                    btn_b.append(
                        [
                            ...
                            InlineKeyboardButton(text="𝐍𝐄𝐗𝐓 ➪", callback_data=f"nextch_{req}_{key}_{offset}")
                        ]
                    )
                else:
                    btn_b.append(
