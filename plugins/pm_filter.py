...
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, SUPPORT_CHAT_ID, CUSTOM_FILE_CAPTION, MSG_ALRT, PICS, AUTH_GROUPS, P_TTI_SHOW_OFF, GRP_LNK, CHNL_LNK, NOR_IMG, LOG_CHANNEL, SPELL_IMG, MAX_B_TN, IMDB, \
    SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, NO_RESULTS_MSG, IS_VERIFY, DOWNLOAD_BUTTON, FILE_GROUP, FILE_GROUP2
...
async def next_page(bot, query):
    ...
    if ENABLE_SHORTLINK and settings['button']:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"[{get_size(file2.file_size)}] {file2.file_name}", url=await get_shortlink(query.message.chat.id, f"https://telegram.me/{temp.U_NAME}?start=files_{file2.file_id}")
                ),
            ]
            for file2 in files
        ]
        ...
        btn = [
            [
                InlineKeyboardButton(
                    text=f"{file2.file_name}", url=await get_shortlink(query.message.chat.id, f"https://telegram.me/{temp.U_NAME}?start=files_{file2.file_id}")
                ),
                InlineKeyboardButton(
                    text=f"{get_size(file.file_size)}",
                    url=await get_shortlink(query.message.chat.id, f"https://telegram.me/{temp.U_NAME}?start=files_{file2.file_id}")
                ),
            ]
            for file2 in files
        ]
...
async def pmnext_page(bot, query):
    ...
    try:
        if settings['max_btn']:
            ...
            if n_offset == 0:
                btn.append(
                    [InlineKeyboardButton("⌫ 𝐁𝐀𝐂𝐊", callback_data=f"pmnext_{req}_{key}_{off_set}"), InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages")]
                )
            elif off_set is None:
                btn.append([InlineKeyboardButton("𝐏𝐀𝐆𝐄", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages"), InlineKeyboardButton("𝐍𝐄𝐗𝐓 ➪", callback_data=f"pmnext_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
            ...
            if n_offset == 0:
                btn.append(
                    [InlineKeyboardButton("⌫ 𝐁𝐀𝐂𝐊", callback_data=f"pmnext_{req}_{key}_{off_set}"), InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total/int(MAX_B_TN))}", callback_data="pages")]
                )
            elif off_set is None:
                btn.append([InlineKeyboardButton("𝐏𝐀𝐆𝐄", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total/int(MAX_B_TN))}", callback_data="pages"), InlineKeyboardButton("𝐍𝐄𝐗𝐓 ➪", callback_data=f"pmnext_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
                        InlineKeyboardButton("⌫ 𝐁𝐀𝐂𝐊", callback_data=f"pmnext_{req}_{key}_{off_set}"),
    ...
    if files:
        ...
        if offset != "":
            ...
            try:
                if settings['max_btn']:
                    btn.append(
                        [InlineKeyboardButton("𝐏𝐀𝐆𝐄", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/10)}", callback_data="pages"), InlineKeyboardButton(text="𝐍𝐄𝐗𝐓 ➪", callback_data=f"pmnext_{req}_{key}_{offset}")]
                    )
                else:
                    btn.append(
                        [InlineKeyboardButton("𝐏𝐀𝐆𝐄", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/int(MAX_B_TN))}", callback_data="pages"), InlineKeyboardButton(text="𝐍𝐄𝐗𝐓 ➪", callback_data=f"pmnext_{req}_{key}_{offset}")]
                    )
            except KeyError:
                await save_group_settings(query.message.chat.id, 'max_btn', True)
                btn.append(
...
async def select_language(bot, query):
    ...
    btn = [[
        ...
        InlineKeyboardButton("Gold Channel", callback_data=f"lang#{userid}#gc")
    ],[
        InlineKeyboardButton("One Channel", callback_data=f"lang#{userid}#One"),
        InlineKeyboardButton("Happy Channel", callback_data=f"lang#{userid}#hc")
    ],[
...
async def cb_handler(client: Client, query: CallbackQuery):
        ...
        if CUSTOM_FILE_CAPTION:
            try:
                f_caption = CUSTOM_FILE_CAPTION.format(file_name='' if title is None else title,
                                                       ...
                                                       user_link='' if user is None else user)
            except Exception as e:
                logger.exception(e)
            f_caption = f_caption
    ...
    if query.data.startswith("file2"):
        ...
        caption1 = f"⚠️{query.from_user.mention} \n\nအချောလေး ရှာတဲ့  {title} ဇာတ်ကား အဆင့်သင့်ပါ ⬇️ "
        await client.send_message(
             chat_id=query.from_user.id,
             text=caption1,
             reply_markup=InlineKeyboardMarkup(
                    [
                        [InlineKeyboardButton('Join Database link', url="https://t.me/+6Rq1ZLh5UExiNTUx")],
                        [InlineKeyboardButton(f'📥 {title} 📥', url=file_send.link)]
                    ]
             )
        )
        await asyncio.sleep(600)
