...
from info import CHANNELS, ADMINS, AUTH_CHANNEL, LOG_CHANNEL, PICS, BATCH_FILE_CAPTION, CUSTOM_FILE_CAPTION, CUSTOM_FILE_CAPTION2, PROTECT_CONTENT, CHNL_LNK, GRP_LNK, REQST_CHANNEL, SUPPORT_CHAT_ID, MAX_B_TN, IS_VERIFY, DOWNLOAD_BUTTON, FILE_GROUP
...
async def start(client, message):
        ...
        try:
            ...
            await client.send_message(
                 ...
                 reply_markup=InlineKeyboardMarkup(
                        [
                            [InlineKeyboardButton('Join Database link', url="https://t.me/+6Rq1ZLh5UExiNTUx")],
                            [InlineKeyboardButton(f'📥 {files_.file_name} 📥', url=msg.link)]
                        ]
                 )
            )
            await asyncio.sleep(600)
            ...
            if CUSTOM_FILE_CAPTION2:
                try:
                    f_caption=CUSTOM_FILE_CAPTION2.format(file_name= '' if title is None else title, file_size='' if size is None else size, file_caption='')
                except:
                    return
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            f_caption=CUSTOM_FILE_CAPTION2.format(file_name= '' if title is None else title, file_size='' if size is None else size, file_caption='' if f_caption is None else f_caption)
        except Exception as e:
            logger.exception(e)
            f_caption=f_caption
    if f_caption is None:
...
@Client.on_message(filters.command('delete2') & filters.user(ADMINS))
async def delete2(bot, message):
    """Delete file from database"""
    reply = message.reply_to_message
    if reply and reply.media:
        msg = await message.reply("Pʀᴏᴄᴇssɪɴɢ...⏳", quote=True)
